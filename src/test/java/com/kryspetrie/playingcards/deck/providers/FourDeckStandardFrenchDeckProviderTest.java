...
class FourDeckStandardFrenchDeckProviderTest {
    ...
    private final DeckProvider provider = new FourDeckStandardFrenchDeckProvider();

    @Test
    void testDeckHasFourInstancesOfExpectedCards() {
        final var deck = provider.provideDeck();

        // We should have four instances of all cards
        for (final Card expectedCard : CARDS_IN_DECK) {
            final var cardMatches = deck.getCardStream().filter(expectedCard::equals).count();
            Assertions.assertThat(cardMatches).isEqualTo(4);
        }
    }
}